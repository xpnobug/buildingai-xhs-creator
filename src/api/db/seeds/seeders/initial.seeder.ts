...
export class XhsCreatorInitialSeeder extends BaseSeeder {
    ...
    async run(dataSource: DataSource): Promise<void> {
        ...
        try {
            ...
            await queryRunner.query(
                `ALTER TABLE "buildingai_xhs_creator"."xhs_config" 
                 ADD COLUMN IF NOT EXISTS "home_title" character varying(500) NOT NULL DEFAULT '今天想在无限画布创作什么？'`,
            );
            await queryRunner.query(
                `COMMENT ON COLUMN "buildingai_xhs_creator"."xhs_config"."home_title" IS '首页标题文案'`,
            );
            await queryRunner.query(
                `ALTER TABLE "buildingai_xhs_creator"."xhs_config" 
                 ADD COLUMN IF NOT EXISTS "quick_start_templates" jsonb`,
            );
            await queryRunner.query(
                `COMMENT ON COLUMN "buildingai_xhs_creator"."xhs_config"."quick_start_templates" IS '快速开始模板列表 [{title, image}]'`,
            );
            await queryRunner.query(
                `ALTER TABLE "buildingai_xhs_creator"."xhs_config" 
            ...
            await queryRunner.query(
                `COMMENT ON COLUMN "buildingai_xhs_creator"."xhs_config"."outline_prompt" IS '大纲生成提示词模板，留空使用默认'`,
            );
            await queryRunner.query(
                `ALTER TABLE "buildingai_xhs_creator"."xhs_config" 
            ...
            await queryRunner.query(
                `COMMENT ON COLUMN "buildingai_xhs_creator"."xhs_config"."image_prompt" IS '图片生成提示词模板，留空使用默认'`,
            );
            await queryRunner.query(
            ...
            await queryRunner.query(
                `INSERT INTO "buildingai_xhs_creator"."xhs_config"
                    ("plugin_name", "bind_key_config_id", "text_key_config_id", "image_key_config_id", "cover_image_power", "content_image_power", "text_model", "text_model_id", "image_model", "image_model_id", "image_endpoint_type", "image_endpoint_url", "high_concurrency", "outline_power", "free_usage_limit", "home_title", "quick_start_templates")
                 VALUES ('小红书图文生成', '', '', '', 80, 40, 'gpt-4o-mini', NULL, 'gpt-image-1', NULL, 'images', NULL, false, 10, 5, '今天想在无限画布创作什么？', NULL)
