...
export class OutlineService {
    ...
    async generateOutline(
        ...
        // 构建提示词（与 RedInk 逻辑一致）
        const prompt = this.buildOutlinePrompt(topic, userImages);
    ...
    /**
     * 构建大纲生成的提示词（使用与 RedInk 相同的 outline_prompt 模板）
     */
    private buildOutlinePrompt(topic: string, userImages?: string[]): string {
        let prompt = this.outlinePromptTemplate.replace("{topic}", topic);
